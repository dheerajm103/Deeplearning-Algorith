...
df = pd.read_csv("50_Startups.csv")          # importing data set
...
df=pd.get_dummies(df,drop_first=(True))      # dummy column for categorical column  
...
#predictors
x =df.loc[:, df.columns != 'Profit'].values.astype("float32")   
#target
y= df["Profit"].values.astype("float32")                        
...
history = model.fit(x_train_scaled, y_train, epochs=100, batch_size=20,validation_split=0.3,shuffle=False)
